...
def enumerate_group_elements(group, verbosity=False):
    """
...
def acceptable_hilbert_symbol_pair(a,b):
    """
    Returns True if a and b generate and irreducible subgroup and a is not parabolic.
    Returns False otherwise. As a word of caution, note that this is only about group
    elements, not actual entries of the Hilbert symbol. Also note that this doesn't
    explictly check whether b is parabolic. This may be a slight overfactor in this
    module alone, but this function is actually used in other places. Ok, upon
    further reflection this may be unncessary in total and perhaps should be deleted.
    """
    if not is_parabolic(a) and not generate_reducible_subgroup(a,b): return True
    return False

def find_hilbert_symbol_elements(group):
    """
    ...
    traces of the group. We return the pair as ApproximateAlgebraicNumbers.
    """
    gen = enumerate_group_elements(group)
    while True:
        elt1 = next(gen)
        elt2 = next(gen)
        if not is_parabolic(elt1) and not generate_reducible_subgroup(elt1, elt2):
            return (elt1.trace()**2-2, elt1.commutator(elt2)-2)
        if not is_parabolic(elt2) and not generate_reducible_subgroup(elt1, elt2):
            return (elt2.trace()**2-2, elt2.commutator(elt1)-2)
