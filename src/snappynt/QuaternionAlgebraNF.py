...
will be for quaternion algebras associted to Kleinian groups, so there might are a few
other methods which are attuned to this purpose.

For now, the class QuaternionAlgebraNF doesn't incorporate the full and rich structure of
...
ramification data, but this module doesn't provide that functionality. To that end one
should take caution in using certain sage methods that this class inherits. For example,
sage provides a way to base extend, but doing so then asking for ramification data will
currently produce an incorrect answer because base changing doesn't remove previously
computed ramification information.

This class basically doesn't work when the base ring is QQ amusingly. The issue is many
number field methods in sage are not implemented for QQ. One should use the usual
QuaternionAlgebra class from sage in this case.
"""
